...
* **Three.js / WebGL**: Creación de mallas (Meshes), texturas dinámicas generadas proceduralmente mediante Canvas API, iluminación direccional y shaders básicos (mar de lava animado).
* **HTML5 & CSS3**: Arquitectura modular con estricta separación de responsabilidades, interfaces de usuario (UI) superpuestas sobre el lienzo 3D, animaciones nativas por CSS (`@keyframes`) y Flexbox.
* **UX / Game Design**: Implementación de mecánicas avanzadas como *Coyote Time* para saltos precisos, multiplicador de impulso basado en inputs de teclado y "Death Zones" dinámicas.

### Instalación y Uso Local
1. Clona este repositorio.
2. Ejecuta `npm install` para instalar el servidor estático ligero.
...
* **Three.js / WebGL**: Mesh creation, procedurally generated dynamic textures using the Canvas API, directional lighting, and basic shaders (animated lava sea).
* **HTML5 & CSS3**: Modular architecture with strict separation of concerns, overlapping User Interfaces (UI) over the 3D canvas, native CSS animations (`@keyframes`), and Flexbox styling.
* **UX / Game Design**: Implementation of advanced platforming mechanics such as *Coyote Time* for precise jumps, an impulse multiplier based on keyboard inputs, and dynamic "Death Zones".
